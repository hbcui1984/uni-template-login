...
var passwordSecret = "your-password-sec";
var tokenSecret = "your-token-sec";
...
async function register(user, context) {
  ...
  if (user.email && user.email.trim()) {
    ...
    const emailInDB = await userCollection.where({
      email: user.email
    }).count();

    log('emailInDB:', emailInDB);

    if (emailInDB && emailInDB.total > 0) {
      return {
        code: 1002,
        msg: '邮箱已存在'
      }
  ...
  if (userInDB && userInDB.data && userInDB.data.length > 0) {
    ...
    if (encryptPwd(user.password) === pwdInDB) {
      try {
        ...
        const token = uniToken.createToken(userInDB.data[0]);
        log('token', token);
